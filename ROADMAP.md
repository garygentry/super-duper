...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP9-large-drive-acceptance`; SOP9a accepted | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | Consume only fixed `sop9b-representative-cancellation-v1` on D: under the accepted write-once/no-favorable-retry runner. Retain every outcome and cleanup fact, then verify/document/commit SOP9b before any single- or multi-drive arm. Preserve SOP6/SOP7 policies, both SOP8 policy values and immutable history, exact duplicate/hard-link behavior, telemetry/memory guarantees, retained evidence, the honest unevaluated SOP2 residual risk, and every production lock. Do not substitute parked release validation. |
