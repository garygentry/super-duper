...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP6-device-aware-scheduler` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP5 are accepted. Exact-size singleton buckets now resolve from discovery metadata with zero partial/full content opens; existing counters report singleton files/bytes as metadata-resolved, admit only multi-file buckets as hash candidates, and preserve actual physical I/O. Debug/Release Rust and Windows matrices and production-lock checks pass. The next session may advance only to `SOP6-device-aware-scheduler`; do not begin SOP7+, another representative/physical campaign without its gate authority, or parked release validation. |
