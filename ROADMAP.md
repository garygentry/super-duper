...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP9-large-drive-acceptance`; SOP9a-SOP9b accepted | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | Consume only fixed `sop9c-single-drive-reference-repeat-v1` on E: under the accepted write-once/no-favorable-retry runner. Retain its forced/reuse arms and every cleanup fact, then verify/document/commit SOP9c before the multi-drive arm. Preserve SOP6/SOP7 policies, both SOP8 policy values and immutable history, exact duplicate/hard-link behavior, telemetry/memory guarantees, retained evidence, the honest unevaluated SOP2 residual risk, and every production lock. Do not substitute parked release validation. |
