...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP9-large-drive-acceptance` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP9 is split into five fixed packages. Implement only `SOP9a-write-once-campaign-tooling`; no representative scan is authorized until its write-once/failure-retention/verifier contract accepts. Later fixed evidence packages are representative cancellation, single-drive forced/reuse, multi-drive forced/reuse, and final acceptance. Preserve SOP6/SOP7 policies, both SOP8 policy values and immutable history, exact duplicate/hard-link behavior, cancellation/telemetry/memory guarantees, retained evidence, the honest unevaluated SOP2 residual risk, and every production lock. Do not substitute parked release validation. |
