...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP5-skip-singleton-size-buckets` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 and SOP4 are accepted. The Performance tab now uses query-only fixed status summaries, hard 25-run/six-phase/64-device Core/WPF bounds, explicit unavailable states, current/peak drive metrics, and context-qualified retained-run comparison with keyboard, automation, focus, system-brush, restart, and loaded-STA evidence. The next session may advance only to `SOP5-skip-singleton-size-buckets`; do not begin SOP6+, another representative/physical campaign, or parked release validation. |
