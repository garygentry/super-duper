...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP7-hash-read-path` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP6 and SOP7a are accepted. The SOP7 harness now isolates path locality, bucket order, buffer size, sequential read-ahead, and partial-prefix reuse one factor at a time while preserving the selected SOP6 device reader ceilings and complete write-once evidence. Advance only to `SOP7b-path-locality`; do not begin SOP8+, another physical campaign outside SOP7's exact authority, or parked release validation. |
