...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP7-hash-read-path` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP6 and SOP7a-c are accepted. Path locality was rejected, while descending exact-size buckets improved direct-read mean wall time 3.46% on SSD and 8.43% on HDD and are now the production default with full Rust regression passing. Advance only to `SOP7d-buffer-read-ahead`; do not begin SOP8+, another physical campaign outside SOP7's exact authority, or parked release validation. |
