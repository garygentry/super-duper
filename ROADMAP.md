...
|---|---|---|---|
| Windows UI redesign | Active on `codex/ui-redesign`; direction accepted with long-scan/rescan requirements | [`plans/ui-redesign/README.md`](plans/ui-redesign/README.md) and [`plans/ui-redesign/execution-plan.md`](plans/ui-redesign/execution-plan.md) | `UIR-06a`: dedicated Review overview, bounded Files/Folders review queries and revision-aware non-deleting validation status; UIR-03 complete and UIR-04a/b/c plus UIR-05a/b/c locally implemented, with later integration/operator validation retained. Use the compact session checkpoint and kickoff prompt. Baseline preserved on `wpf-poc` at `deefa40`. No production deletion enablement or consumed-campaign rerun. |
