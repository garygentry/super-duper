...
|---|---|---|---|
| Large-drive scan optimization and observability | Blocked at `SOP9c-single-drive-reference-repeat`; SOP9a-SOP9b accepted, SOP9c V1 retained invalid | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | The fixed SOP9c V1 identity is consumed: its forced E: arm entered persistence, then the V1 180-second frame deadline force-stopped the worker and preserved diagnostic state before any terminal snapshot or reuse arm. Do not rerun/overwrite V1 or start SOP9d. A separately reviewed V2 protocol and new fixed identity require explicit operator authority. Preserve SOP6/SOP7 policies, both SOP8 policy values and immutable history, exact duplicate/hard-link behavior, telemetry/memory guarantees, all retained evidence, the honestly unevaluated SOP2 residual risk, and every production lock. Do not substitute parked release validation. |
