...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP7-hash-read-path` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP6 and SOP7a-d are accepted. Production now admits descending buckets, a measured 1 MiB SSD/64 KiB rotational-or-unknown full-read buffer, and the Windows sequential-read hint; path locality was rejected. Full serial Debug/Release Rust passes. Advance only to `SOP7e-partial-prefix-reuse`; do not begin SOP8+, another physical campaign outside SOP7's exact authority, or parked release validation. |
