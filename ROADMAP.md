...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP8-repeat-run-cache` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP7 and SOP8a-d are accepted. The sole SOP8d evidence selects `reuse_verified`; implement only `SOP8e-session-policy-and-acceptance` to carry that default and the forced alternate through immutable run truth and the accessible Setup choice. Preserve SOP6/SOP7 policies, exact duplicate/hard-link behavior, cancellation, telemetry/memory bounds, retained cache evidence, and every production lock. Do not start SOP9, unrelated campaigns, or parked release validation. |
