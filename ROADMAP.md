...
|---|---|---|---|
| Large-drive scan optimization and observability | Active; blocked at operator decision | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | Functional evidence and the fixed-cost short leg pass. V1 stopped before any arm. The sole authorized [`SOP2f-representative-v2`](docs/scan-progress-representative-protocol-v2.md) invocation completed setup but control warmup 0 did not complete within its bound; an empty-aggregate defect then prevented native invalid-evidence serialization. The recovered write-once incident retains zero completed/measured arms and no aggregate, consumes v2 authority, and blocks rerun. Explicitly authorize design of a new versioned protocol, or waive/reject SOP2; SOP2 remains unaccepted. |
