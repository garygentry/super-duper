...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP10f-release-scale-acceptance`; implementation packages SOP10b through SOP10e are accepted, and scale v1 is retained invalid on an evidence-query defect | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) and [`docs/scan-optimization-kickoff-prompt.md`](docs/scan-optimization-kickoff-prompt.md) | Run exactly one separately versioned v2 scale attempt after the query-only repair, then full matrices and fixed Release packaging. Do not reuse v1, rerun consumed SOP9c identities, or start a physical full-drive scan without separate explicit approval. |
