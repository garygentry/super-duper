...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP4-performance-tab` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. `SOP3a` through `SOP3d` are accepted: exact durable warning accounting now has one bounded active/completed History drilldown, a count-aware Alt+W Progress entry, separate supplemental diagnostic-log disclosure, terminal/restart continuity, stable focus/automation, system brushes, and latest-only announcements. The next session may advance only to `SOP4-performance-tab`; do not begin SOP5+, another representative campaign, or parked release validation. |
