...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP10-large-folder-analysis-remediation`; implementation packages SOP10b through SOP10e are accepted | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) and [`docs/scan-optimization-kickoff-prompt.md`](docs/scan-optimization-kickoff-prompt.md) | Execute `SOP10f-release-scale-acceptance`: retain the first deterministic scale result, run full matrices, and package the fixed Release build/checklist. Do not rerun consumed SOP9c identities or start a physical full-drive scan without separate explicit approval. |
