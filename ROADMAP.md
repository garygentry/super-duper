...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP3-current-warning-log` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 is `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. `SOP3a` through `SOP3c` are accepted: live counts are durably accounted, warning pages are revision/state-bound, and one reusable Core drilldown now owns bounded active/completed paging plus the one-way terminal latch. Advance only to `SOP3d-accessible-warning-entry-and-acceptance`; do not begin SOP4, another representative campaign, or parked release validation. |
