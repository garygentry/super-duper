...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP7-hash-read-path` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP6 and SOP7a-b are accepted. Retained path-locality evidence rejects a product change: the cached SSD workload regressed 11.57%, and an idealized direct rotational workload improved only 1.21%, so production insertion order is unchanged. Advance only to `SOP7c-bucket-ordering`; do not begin SOP8+, another physical campaign outside SOP7's exact authority, or parked release validation. |
