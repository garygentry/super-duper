...
|---|---|---|---|
| Large-drive scan optimization and observability | Blocked at the SOP9c V2 operator-execution boundary; SOP9a-SOP9b accepted, SOP9c V1 retained invalid, V2 design accepted | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | The V2 protocol is committed but unconsumed under fixed identity `sop9c-single-drive-reference-repeat-v2`; obtain separate explicit operator approval for exactly one invocation. Its one-pending-read persistence watchdog changes only V1's causal 180-second quiet-frame defect. Do not rerun/overwrite V1, alter its preserved diagnostics, execute V2, or start SOP9d without the named authority. Preserve SOP6/SOP7 policies, both SOP8 policy values and immutable history, exact duplicate/hard-link behavior, telemetry/memory guarantees, all retained evidence, the honestly unevaluated SOP2 residual risk, and every production lock. Do not substitute parked release validation. |
