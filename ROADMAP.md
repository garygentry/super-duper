...
|---|---|---|---|
| Large-drive scan optimization and observability | Blocked after the consumed invalid SOP9c V2 attempt; SOP9a-SOP9b accepted, SOP9c V1/V2 retained invalid | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | The sole authorized `sop9c-single-drive-reference-repeat-v2` invocation consumed its identity, passed the fixed physical preflight, created scoped state, and completed both pinned builds, but its campaign host ended before worker start or native evidence finalization. A guarded second admission was refused; zero arms, results, or measurements exist, and the post-exit audit found no worker or scoped V2 state. Do not rerun V1/V2, invent a causal product defect, alter retained evidence/V1 diagnostics, or start SOP9d. A successor design and identity require a separately reviewed causal defect plus explicit operator authority. Preserve SOP6/SOP7 policies, both SOP8 policy values and immutable history, exact duplicate/hard-link behavior, telemetry/memory guarantees, the honestly unevaluated SOP2 residual risk, and every production lock. Do not substitute parked release validation. |
