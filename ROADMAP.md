...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP7-hash-read-path` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP6 are accepted. Hashing now uses bounded physical-device queues instead of nested global reads: one reader for rotational/unknown mapping, four for SSD, a global logical-parallelism ceiling, and independent progress on separate devices. Retained 1/N/N/1 direct-I/O HDD/SSD evidence selects those defaults with exact checksums and I/O; full Debug/Release Rust and Windows matrices and production locks pass. The next session may advance only to `SOP7-hash-read-path`; do not begin SOP8+, another physical campaign outside SOP7's exact authority, or parked release validation. |
