...
|---|---|---|---|
| Large-drive scan optimization and observability | SOP8 accepted; stopped before `SOP9-large-drive-acceptance` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP8 are accepted. `reuse_verified` is the measured shipped default, `revalidate_content` is the explicit alternate, and immutable history preserves both new and legacy truth. SOP9 is dependency-ready but not started; audit and authorize its representative operator evidence separately. Preserve all accepted policies, retained evidence, and production locks, and do not substitute the parked release-validation stream. |
