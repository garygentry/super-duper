...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP10-large-folder-analysis-remediation`; streaming exact-folder analysis and exact-only product behavior are accepted | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) and [`docs/scan-optimization-kickoff-prompt.md`](docs/scan-optimization-kickoff-prompt.md) | Implement `SOP10d-scan-resistant-qualified-cache`, then the dependency-ready progress and synthetic Release-scale packages. Do not rerun consumed SOP9c identities or start a physical full-drive scan without a separate explicit approval. |
