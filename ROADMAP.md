...
|---|---|---|---|
| Large-drive scan optimization and observability | Active at `SOP8-repeat-run-cache` | [`docs/scan-optimization-plan.md`](docs/scan-optimization-plan.md) | SOP2 remains `accepted_with_operator_waiver`; its unmeasured representative-overhead risk remains assigned to SOP9. SOP3 through SOP7 and SOP8a-c are accepted. Execute only the predeclared `SOP8d-repeat-policy-measurement`; forced revalidation remains the provisional default and Windows rename reuse remains conservatively rejected. Preserve SOP6/SOP7 read policies, exact duplicate/hard-link behavior, cancellation, telemetry/memory bounds, cache evidence, and every production lock. Do not start UI, SOP9, unrelated physical campaigns, or parked release validation. |
