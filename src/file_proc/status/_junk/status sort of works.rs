...
fn get_pb_bar() -> ProgressBar {
    ...
    pb.set_style(
        ProgressStyle::with_template(
            &format!(
                "[{{elapsed_precise}}] {{prefix:.bold}}▕{{bar:.{}}}▏{{msg}}",
                String::from("blue")
            )
        )
            .unwrap()
    ...
    for message in rx {
        ...
        match message {
            StatusMessage::ProcessBegin => {
                ...
                let message = format!("Processing {}, {}", HumanDuration(pb_scan.elapsed()), i);
                pb_stats.set_message(message);
            }
            ...
            StatusMessage::CacheToDupeBegin => {
                pb_cache_to_dupe_proc_bar.set_prefix("Cache to duping...");
            ...
            StatusMessage::CacheToDupeEnd => {
                pb_cache_to_dupe_proc_bar.finish_with_message(
                    "Dupe Files Vec created successfully."
                );
            }
            ...
            StatusMessage::DbDupeFileInsertEnd => {
                ...
                pb_db_dupe_file_proc_bar.set_length(0);
