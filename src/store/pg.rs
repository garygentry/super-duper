...
pub fn truncate_dupe_file_part() {
    truncate_table("dupe_file_part");
}

pub fn truncate_dedupe_session() {
    truncate_table("dedupe_session");
}
...
fn truncate_table(table_name: &str) {
    ...
    diesel
        ::sql_query(
            format!("TRUNCATE TABLE {} RESTART IDENTITY CASCADE", table_name)
        )
