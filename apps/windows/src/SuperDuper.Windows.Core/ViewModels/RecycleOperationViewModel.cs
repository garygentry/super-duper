...
public sealed class RecycleOperationViewModel : ObservableObject, IDisposable
{
    ...
    public string RecoveryEvidenceSummary => Operation?.Status == "recovery_required"
        ? $"Operation key: {Operation.OperationId}\n"
          ...
          + $"Policy version: {Operation.PolicyVersion}\n"
          + $"Preflight snapshot signature: {Operation.PreflightSnapshotSignature}\n"
          + $"Intent signature: {Operation.IntentSignature}\n"
          + $"Status: {Operation.Status}\n"
          + $"Prepared at: {Operation.PreparedAt}\n"
          + $"Submitted at: {Operation.SubmittedAt ?? "none recorded"}\n"
          + $"Completed at: {Operation.CompletedAt ?? "none recorded"}\n"
          + $"Cancellation requested: {Operation.CancellationRequested.ToString().ToLowerInvariant()}\n"
