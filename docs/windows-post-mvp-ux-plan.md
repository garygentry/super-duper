...
- An acknowledged `shell_started` batch without a complete durable report becomes `ambiguous`; its
  pending items become `unknown`; and its operation becomes terminal `recovery_required`.
- The original operation, batch, item, report, and recovery rows preserve operation/run/preflight/
  revision IDs, signatures, lifecycle times, source-snapshot identity/size/time, batch/Shell-attempt
  correlation, stable reason codes, numeric Shell HRESULTs, and recorded recycled-item presence.
- WPF reconstructs only stored evidence, pages every unknown item, supports exact retry of failed
  read-only loads, and explicitly says not to retry the operation. It does not inspect source or
  Recycle Bin state, resolve an item, or change the original result.
- Path absence, path presence, a missing Recycle Bin entry, or a same-named entry is not proof of
...
- `observed_at_source` — the operator identified the expected source item;
- `observed_in_both` — both were observed, so replacement/copy/alias ambiguity remains;
...
behalf. There would be no retry/resubmit/resolve-as-success/resolve-as-failure, restore, permanent-
delete, evidence-clear, or live-state mutation action.

If an operator wants to reconsider an item still visible at its source, the only deletion path
would start from a new scan snapshot, new review decision, new preflight, and new operation ID. No
old operation, batch, Shell attempt, or item may be replayed or copied forward as execution
authority. Exact-folder unknowns remain whole-item ambiguous even if some descendants are observed;
...
1. **Option B — preserve-only acknowledgement.** Keep the existing read-only reconstruction and
   ...
   smallest and safest product surface, but it cannot account for per-item review, gives weak
   restart audit value, and may be insufficient for operator acceptance of a production-enabled
   workflow.
2. **Option C — automatic source/Recycle Bin inspection and classification.** The app would query
   live source paths or Recycle Bin contents and infer recycled/not-recycled results. This reduces
   operator effort but is not recommended: absence/presence is not authoritative, lookup can race
   external changes, provider inspection can violate no-hydration boundaries, and reclassification
   would overstate the durable Shell evidence.
3. **Option D — retry or replay unknown work.** The app would resubmit the old item/batch/operation.
   This is not recommended and conflicts with the accepted safety contract because an unknown item
   may already have been recycled; repeating it could act on a replacement path or duplicate a
   completed mutation.
4. **Option E — overwrite `unknown` with an operator-selected success/failure.** This is simpler to
   display than a separate review record but is not recommended because it erases the distinction
...
| 2026-08-22 | Made failed forward recovery paging transactional with a retry regression. | Retain the last committed page, cursor history, and navigation availability when a current worker fetch fails; announce the page error and let retry request the same page without enabling execution, filesystem inspection, replay, or recovery resolution. All physical/provider/performance and recovery-design gates remain open. |
| 2026-08-22 | Covered failed backward recovery paging after bounded-cache eviction. | Prove the committed page and both navigation directions survive a worker failure and exact retry when an older page must be fetched again; keep execution, filesystem inspection, replay, and recovery resolution disabled. All physical/provider/performance and recovery-design gates remain open. |
| 2026-08-22 | Cleared stale Recycle Bin automation announcement text when the selected run context changes. | Prevent a prior operation or recovery message from remaining attached to an empty/non-completed replacement context without emitting a misleading new notification. Physical accessibility and all recovery/provider/performance/design gates remain open. |
| 2026-08-22 | Split read-only Recycle Bin failures from successful result notifications with Core and STA automation coverage. | Raise operation and paging failures through an assertive `ActionAborted`/`ImportantMostRecent` channel while committed reconstruction and page loads remain repeatable `ActionCompleted` notifications. Physical accessibility and all recovery/provider/performance/design gates remain open. |
| 2026-08-22 | Exercised both read-only Recycle Bin notification channels through loaded UI Automation peers. | Give the successful-results channel a stable automation ID and prove that loaded WPF emits its `ActionCompleted` event separately from the error channel's assertive `ActionAborted` event. This is automated contract evidence only; physical accessibility and all recovery/provider/performance/design gates remain open. |
| 2026-08-22 | Included the first committed unknown-result range in the Recycle Bin reconstruction notification. | Make initial recovery reconstruction and later page navigation announce the same exact bounded range/total contract without filesystem inspection, replay, resolution, executor wiring, or submission. Physical accessibility and all recovery/provider/performance/design gates remain open. |
| 2026-08-22 | Added exact read-only retry for failed Recycle Bin detail-page loads, including the initial recovery page. | Preserve the durable operation summary and exact failed cursor, expose an explicitly non-operation retry, and announce only the successfully committed retry page. Execution, filesystem inspection, replay, resolution, and all physical/provider/performance/design gates remain unchanged. |
| 2026-08-22 | Made successful Recycle Bin detail-page retries commit their matching cursor-history transition. | Keep Previous/Next availability and behavior consistent after exact forward or backward retry, including cache-evicted pages, without changing operation state, inspecting the filesystem, replaying or resolving recovery, enabling execution, or closing physical/provider/performance/design gates. |
| 2026-08-22 | Extended the read-only Recycle Bin retry affordance to a failed operation-summary reconstruction request. | Retry the exact selected completed-run read when no durable operation summary was returned, preserve generation/cancellation rejection so a late retry cannot replace a newer context, and keep execution, filesystem inspection, replay, recovery resolution, and every physical/provider/performance/design gate unchanged. |
| 2026-08-22 | Cleared stale assertive Recycle Bin page-error text after a successfully committed exact retry. | Keep the successful result announcement separate from the resolved error channel without emitting a misleading second assertive notification; execution, filesystem inspection, replay, recovery resolution, and every physical/provider/performance/design gate remain unchanged. |
| 2026-08-22 | Covered late failure of an exact read-only Recycle Bin page retry after the selected run changes. | Lock the generation boundary so the stale failure cannot replace the newer committed page, publish an announcement or error, expose a retry, or mutate cursor history; execution, filesystem inspection, replay, recovery resolution, and every physical/provider/performance/design gate remain unchanged. |
