...
| `SOP2f-progress-acceptance` | `in_progress` | SOP2b, SOP2c, SOP2d, SOP2e | Integrate and accept the gate with one focused cross-layer verifier and proportionate full matrices. | Functional verification must pass. The operator-approved two-part overhead budget additionally requires: (1) positive median treatment-minus-control overhead on the unchanged short fixture no greater than 100,000,000 ns wall and 125,000,000 ns worker CPU; and (2) one predeclared representative-duration profile whose all-run aggregate wall and worker CPU overhead are each strictly below 100 bp. Negative deltas mean no detected positive overhead, never acceleration. Both retained results are required without retry-to-green. | Functional acceptance is complete: the focused verifier passes 11 exact Rust, 5 Infrastructure, 27 Core, and 1 loaded-STA WPF tests; full Debug and Release Rust each pass 175 with 5 ignored; serialized Debug and isolated Release Windows matrices each pass 137 Core, 74 Infrastructure, and 3 smoke with the same 5 operator-only skips; real Debug and isolated-publish Release worker/WPF smoke pass. The retained [short cross-revision Release profile](evidence/scan-progress-overhead-20260825.json) now passes the approved fixed caps: +66.9243 ms wall and -109.375 ms CPU (no detected positive CPU cost). The representative profile is authorized exactly once between `0a3c1c1` and `f803cbd`: a deterministic warm-filesystem-cache/cold-application-state fixture of 600,000 allocated 4,096-byte unique-content files plus four allocated large identical pairs at 268,435,456, 268,500,992, 268,566,528, and 268,632,064 bytes (600,008 files/4,605,870,080 bytes total). Content uses generator v1 with SplitMix64 seed `0x534F503246524550`; the fixture manifest/count/bytes/allocation/content relationships are validated before timing and never changed from observed duration. One C then T warmup precedes five measured runs per revision in `C,T / T,C / C,T / T,C / C,T` order; every arm receives revision-neutral full-file conditioning, a new worker, and fresh product/status/hash state. Every warmup and measured run must complete in 60-600 seconds; all five runs feed aggregate ratios with no exclusions. Correctness, terminal ordering, progress-path exercise, free-space, write-once evidence, and safe-cleanup checks fail closed. Either aggregate at or above 100 bp fails. No retry, resize, outlier removal, order/cache change, or adjacent profile is authorized. |
...
- Advance through dependency-ready work packages until a real stop condition in the resumable
  execution protocol applies. `SOP2f-progress-acceptance` is in progress for exactly the approved
  representative-duration profile; do not advance to SOP3 or reopen
