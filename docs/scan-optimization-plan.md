...
- last accepted work package: `SOP8c-partial-full-cache-integration`;
...
| `SOP8d-repeat-policy-measurement` | `ready` | SOP8c | Retain one write-once Release comparison on a fixed 512 MiB collision-heavy local fixture, in forced/reuse/reuse/forced order, with one warm same-process reuse arm and one reopened-store cross-session arm. Select the default from all samples; if verified reuse does not save exact expected reads and positive wall time, keep forced revalidation as default and record reuse as rejected or narrower scope. | Evidence records build/input/store/policy/signature/media/host identity, arm order, partial/full reads and cache outcomes, wall/CPU/memory/process I/O, exact ordered hashes/results, cancellation, cleanup, and every failure. Both reuse arms must save the declared bytes with identical results; the policy record reports medians/tails and never retries for a favorable sample. No full-drive/SOP9 campaign runs. | The executable Release-only ignored profiler is predeclared: 128 fixed 4 MiB files form 64 exact pairs with one shared 1 KiB prefix; it pins the four-arm order, same-process/reopened ownership, metrics/process/device observations, exact 128 KiB plus 512 MiB reuse savings, result equality, cancellation, cleanup, write-once output, and no-retry decision. Two non-physical fixture/validator tests and strict Core Clippy pass. No campaign has run; the next action is its sole authorized invocation. |
| `SOP8e-session-policy-and-acceptance` | `open` | SOP8d | Carry the selected default and explicit alternate through additive run parameters, worker start validation, Core, and one keyboard-accessible Setup choice with concise reuse/fallback disclosure. Persist the selected value in immutable run history, keep it generation-scoped for start/retry, and close SOP8 with one named verifier. | Protocol/Core/loaded-STA tests prove closed-value rejection, selected-default behavior, exact run-history reconstruction, stale/cancelled start silence, accessible name/help/focus/system-brush behavior, and no dispatcher I/O. The named verifier pins store/signature/measurement policy, SOP6/SOP7 decisions, cache/telemetry/correctness/cancellation/cloud/hard-link/memory regressions, full Debug/Release Rust and Windows matrices, and every production deletion lock. SOP9 and parked validation do not run. | Pending. |
