...
- next boundary: operator authorization to design a new separately versioned representative
  protocol, or explicit waiver/rejection of SOP2; v2 authority is consumed and no campaign is
  authorized;
...
| `SOP2f-progress-acceptance` | `blocked_operator_decision` | SOP2b, SOP2c, SOP2d, SOP2e | Integrate and accept the gate with one focused cross-layer verifier and proportionate full matrices. | Functional verification must pass. The operator-approved two-part overhead budget additionally requires: (1) positive median treatment-minus-control overhead on the unchanged short fixture no greater than 100,000,000 ns wall and 125,000,000 ns worker CPU; and (2) one predeclared representative-duration profile whose all-run aggregate wall and worker CPU overhead are each strictly below 100 bp. Negative deltas mean no detected positive overhead, never acceleration. Both retained results are required without retry-to-green. | Functional acceptance and the retained short leg remain passed. V1 retained [pre-measurement failure](evidence/scan-progress-representative-premeasurement-20260825.json) after its two-hour setup window expired before any arm. The sole authorized [`SOP2f-representative-v2`](scan-progress-representative-protocol-v2.md) invocation completed clean builds, exact fixture creation/validation, and its one initial full-content conditioning pass, proving that v1's redundant setup was removed. Control warmup 0 then did not complete inside the declared 600-second arm envelope. Its invalid path exposed a harness defect: `New-Evidence` accessed a missing `Sum` property for the empty measured-control collection, so native invalid-evidence serialization failed after cleanup and the exact primary exception/in-memory attempt details were lost. The recovered write-once [invalid-campaign incident](evidence/scan-progress-representative-overhead-sop2f-v2.json) contains only provable facts: zero completed/measured arms, null aggregates, an unevaluated <100 bp wall/CPU gate, absent campaign root/processes, and consumed one-run authority. No retry, tuning, threshold change, or v2 reuse is authorized. SOP2f/SOP2 remain unaccepted pending explicit authority to design a new separately versioned protocol or an explicit waiver/rejection. |
...
- `SOP2f-progress-acceptance` remains blocked at an operator decision. The separately versioned v2
  protocol was executed once and is retired invalid; its occupied write-once path forbids reuse. Do
  not run v1 or v2, revise v2, design a successor, advance to SOP3, or reopen the completed SOP2
  source audit without explicit approval.
