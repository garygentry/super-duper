...
- last accepted work package: `SOP8b-qualified-content-signature`;
...
| `SOP8c-partial-full-cache-integration` | `ready` | SOP8b | Integrate verified entries before the partial read and at the full-collision step while preserving SOP6 queues and SOP7 order/buffer/hint/no-prefix policy. `reuse_verified` may satisfy partial and full work; `revalidate_content` bypasses both hit paths. Add additive partial-cache counters and keep partial/full logical outcomes separate from actual bytes read across Rust telemetry, worker projection, and defensive Core parsing. | Injected I/O fixtures prove exact duplicate output, zero false hits after every signature mutation, same/cross-open reuse, conservative Windows rename fallback, hard-link physical de-duplication, corruption/store-error fallback, exact partial/full hit/miss/error/store and byte reconciliation, cancellation before queued/cache work, unchanged reader ceilings, and bounded memory. Debug/Release Rust plus affected worker/Core contracts pass. | Pending. |
| `SOP8d-repeat-policy-measurement` | `open` | SOP8c | Retain one write-once Release comparison on a fixed 512 MiB collision-heavy local fixture, in forced/reuse/reuse/forced order, with one warm same-process reuse arm and one reopened-store cross-session arm. Select the default from all samples; if verified reuse does not save exact expected reads and positive wall time, keep forced revalidation as default and record reuse as rejected or narrower scope. | Evidence records build/input/store/policy/signature/media/host identity, arm order, partial/full reads and cache outcomes, wall/CPU/memory/process I/O, exact ordered hashes/results, cancellation, cleanup, and every failure. Both reuse arms must save the declared bytes with identical results; the policy record reports medians/tails and never retries for a favorable sample. No full-drive/SOP9 campaign runs. | Pending. |
