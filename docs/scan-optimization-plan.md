...
- last accepted work package: `SOP8a-versioned-bounded-cache-store`;
...
| `SOP8b-qualified-content-signature` | `ready` | SOP8a | Add a platform-neutral injected signature seam and Windows/local implementation that binds stable physical-file identity, size, nanosecond modified time, and a content-change token without opening file content. Qualification is fail-closed; the same signature must be observed before and after a cache decision or content read. Rename may reuse by stable identity; hard-link aliases retain discovery's one-physical-file behavior. | Fake-platform tests cover unchanged, rename, hard link, same-size edits with preserved modified time, coarse/zero timestamps, identity reuse/change, unsupported/remote state, and before/after mutation. Real Windows fixture proves a content edit changes the token and a rename preserves identity; unavailable evidence forces I/O. No hash/read-path, UI, schema, or campaign changes. | Pending. |
| `SOP8c-partial-full-cache-integration` | `open` | SOP8b | Integrate verified entries before the partial read and at the full-collision step while preserving SOP6 queues and SOP7 order/buffer/hint/no-prefix policy. `reuse_verified` may satisfy partial and full work; `revalidate_content` bypasses both hit paths. Add additive partial-cache counters and keep partial/full logical outcomes separate from actual bytes read across Rust telemetry, worker projection, and defensive Core parsing. | Injected I/O fixtures prove exact duplicate output, zero false hits after every signature mutation, same/cross-open reuse, rename reuse, hard-link physical de-duplication, corruption/store-error fallback, exact partial/full hit/miss/error/store and byte reconciliation, cancellation before queued/cache work, unchanged reader ceilings, and bounded memory. Debug/Release Rust plus affected worker/Core contracts pass. | Pending. |
