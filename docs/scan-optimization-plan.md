...
- next boundary: operator approval/rejection of proposed `SOP2f-representative-v2`; no campaign is
  authorized;
- last accepted work package: `SOP2e-accessible-progress-surface`;
...
| `SOP2f-progress-acceptance` | `blocked_operator_decision` | SOP2b, SOP2c, SOP2d, SOP2e | Integrate and accept the gate with one focused cross-layer verifier and proportionate full matrices. | Functional verification must pass. The operator-approved two-part overhead budget additionally requires: (1) positive median treatment-minus-control overhead on the unchanged short fixture no greater than 100,000,000 ns wall and 125,000,000 ns worker CPU; and (2) one predeclared representative-duration profile whose all-run aggregate wall and worker CPU overhead are each strictly below 100 bp. Negative deltas mean no detected positive overhead, never acceleration. Both retained results are required without retry-to-green. | Functional acceptance and the retained short leg remain passed. The sole v1 representative attempt retained [pre-measurement failure](evidence/scan-progress-representative-premeasurement-20260825.json) after setup and one initial full-content conditioning pass exhausted its two-hour envelope before any arm; no aggregate or threshold evaluation exists. Proposed [`SOP2f-representative-v2`](scan-progress-representative-protocol-v2.md) keeps revisions `0a3c1c1`/`f803cbd`, the exact 600,008-file/4,605,870,080-byte fixture, one C/T warmup, five counterbalanced measured runs per revision, 60-600-second arm qualification, fresh application/product/status/hash state, exact result/counter reconciliation, all-run strict <100 bp wall/CPU math, write-once invalid/pass/fail evidence, safe cleanup, and no retry/tuning. It removes only the 12 redundant per-arm full-content conditioning passes: one revision-neutral initial pass warms the first arm, then every complete scan rewarms exactly the scan-relevant ranges for the next. A 150-minute setup cap plus twelve maximum 10-minute arms and 15-minute closeout derives 17,100 seconds inside a five-hour watchdog. The executable no-state preflight validates revisions, retained evidence hashes, fixture/order/budget arithmetic, 30 GiB free space, process absence, and the absent future evidence path without campaign state. V2 is proposed, not authorized; SOP2f/SOP2 remain unaccepted pending explicit operator approval of exactly one v2 invocation or explicit waiver/rejection. |

## Work selection and evidence rules

- `SOP2f-progress-acceptance` remains blocked at an operator decision. The separately versioned v2
  protocol is predeclared but not authorized. Do not run v1 or v2, revise v2, advance to SOP3, or
  reopen the completed SOP2 source audit without explicit approval.
